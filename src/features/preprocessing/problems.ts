...
export function createPracticeSet(seed: number): PracticeSet {
  ...
  const mean = shown(data.reduce((sum, item) => sum + item, 0) / data.length, 4);
  const sd = shown(Math.sqrt(data.reduce((sum, item) => sum + (item - mean) ** 2, 0) / data.length), 4);
  const zValues = data.map((item) => shown(zScore(item, mean, sd), 4));
  const scaled = decimalScale(data); const scaledValues = scaled.values.map((item) => shown(item, 4));
  const problems: PreprocessingProblem[] = [
    ...
    { id: 'decimal-scaling', kind: 'decimal-scaling', title: 'Decimal scaling', points: 5, precision: 4, prompt: `Use normalization by decimal scaling to transform all age values: ${data.join(', ')}. Round each answer to 4 decimal places and separate values with commas.`, expected: [scaledValues], solution: [{ label: 'Find j', expression: `max |v| = ${Math.max(...data.map(Math.abs))}`, result: `j = ${scaled.exponent}` }, { label: 'Scale', expression: `v / 10^${scaled.exponent}`, result: scaledValues.join(', ') }, { label: 'Check', expression: 'Every absolute value must be less than 1', result: 'Condition satisfied' }] },
  ];
